...
func RunPsiphon(ctx context.Context, l *slog.Logger, wgBind netip.AddrPort, dir string, localSocksAddr netip.AddrPort, country string, conduit bool) error {
	host := ""
	...
	upstreamURL := ""
	if conduit {
		upstreamURL = fmt.Sprintf("socks5://%s", wgBind)
		l.Info("forced upstream conduit enabled", "upstream", upstreamURL)
	}
