...
// StartProxy spawns a socks5/http proxy.
func StartProxy(ctx context.Context, l *slog.Logger, tnet *netstack.Net, bindAddress netip.AddrPort, bypass string) (netip.AddrPort, error) {
...
func (vt *VirtualTun) generalHandler(req *statute.ProxyRequest) error {
	...
	if vt.Bypass != "" {
		...
		for _, token := range strings.Split(vt.Bypass, ",") {
			rule := strings.TrimSpace(token)
			if rule == "" {
				continue
			}
	...
	if shouldBypass {
		vt.Logger.Info("bypassing proxy for destination", "destination", req.Destination)
